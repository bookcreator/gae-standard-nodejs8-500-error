...
module.exports = (req, res, next) => {
   ...
   if (typeis.is(contentType, 'image/*')) {

      const width = getDim(req.query.width)
      const height = getDim(req.query.height)
      const quality = parseInt(req.query.quality)

      const alterQuality = quality && (typeis.is(contentType, 'image/jpeg') || typeis.is(contentType, 'image/jpg'))

      // See if we need to alter the image
      if ((width || height) || alterQuality) {

         // Can't serve range requests like this anymore
         range = null
         res.removeHeader('Accept-Ranges')
         // Unknown resultant size
         res.removeHeader('Content-Length')

         const converter = sharp().limitInputPixels(false)
         streamOpts.converter = converter

         let eTagSuffix = '?'

         if (width || height) {
            eTagSuffix += `s=${width}x${height}`
            converter
               .resize(width, height)
               .withoutEnlargement(true)
               .min()
            if (alterQuality) eTagSuffix += '&'
         }

         if (alterQuality) {
            eTagSuffix += `q=${quality}`
            converter.jpeg({ quality })
         }

         if (eTag) {
            // Format the eTag and update on response
            eTag = etag(eTag.replace(/^"([^"]+)"$/, '$1') + eTagSuffix)
            res.set('ETag', eTag)
         }
      }
   }
   ...
   pipeStream({
      ...
      query: req.query
   }, stream, streamOpts.converter, res, next)
}
...
const pipeStream = ({ tag, source, query }, inputStream, converter, res, next) => {
   let readStream = inputStream
   if (converter) {
      // Pipe errors through to the new stream
      readStream.on('error', function (err) {
         this.unpipe(converter)
         converter.emit('error', err)
      })
      readStream = readStream.pipe(converter)
   }
   readStream.on('error', err => {
      if (converter && err.message) {
      ...
      if (inputStream !== readStream) inputStream.unpipe(converter)
